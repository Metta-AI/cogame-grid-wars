"""Export accepted ordinary-player decisions from complete Grid Wars episodes."""
...
def export(runs: list[Path], output: Path, revision: str, source: str,
    ...
    for run in runs:
        ...
        for artifact in sorted(run.glob("policy_artifact_*.zip")):
            ...
            if (trajectory["schema_version"] != 1 or trajectory["game"] != "grid-wars"
            ...
            for row in trajectory["decisions"]:
                if row["source"] != source:
                    continue
                example = {
                    "episode_id": f"grid-wars-{seed}-seat-{slot}",
                    "seed": f"grid-wars-{seed}",
    ...
    for split, rows in splits.items():
        ...
        with os.fdopen(os.open(path, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600), "w") as destination:
            destination.write("\n".join(rows) + "\n")
    manifest = {"schema_version": 1, "game": "grid-wars", "source_revision": revision,
                "action_schema_revision": "grid-wars-gwl-v1", "teacher": source,
