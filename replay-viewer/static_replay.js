// Grid Wars static replay shell: fetches the replay named by ?replay=<url>,
...
(function () {
  ...
  function start(module, bytes) {
    ...
    var ok = module._gw_load_replay(ptr, bytes.length);
    module._free(ptr);
    if (!ok) {
      fail(readString(module, module._gw_error_ptr(),
        module._gw_error_len()) || "wasm rejected the replay");
      return;
    }
    var payload = JSON.parse(
      readString(module, module._gw_payload_ptr(),
        module._gw_payload_len())
    );
    var loading = document.getElementById("loading");
    if (loading) loading.style.display = "none";
    document.documentElement.removeAttribute("data-replay-error");
    GridWarsRenderer.attachReplay({
      ...
      terrbar: document.getElementById("terrbar"),
      codepane: document.getElementById("codepane"),
      assetBase: "./assets",
      payload: payload
    });
    // The renderer draws on its own animation frame, and the starter's
    // two-rAF delay can still beat the first drawn frame -- the softmax.com
    // embed then samples an unpainted shell (chorus 3c11c953, 2026-08-24).
    // Poll the attribute renderer.js sets on its first drawn frame instead,
    // bounded, so "ready" always means a picture.
    var waited = 0;
    (function awaitFirstFrame() {
      ...
      waited += 1;
      if (waited > 240) {
        tell("error", "renderer never drew a frame");
        return;
      }
  ...
  function load() {
    ...
    caption(attempt > 1 ? "RETRYING REPLAY… (attempt " + attempt + ")" :
      "LOADING REPLAY…");
    if (!modulePromise) {
      modulePromise = GridWarsReplayModule().catch(function (error) {
