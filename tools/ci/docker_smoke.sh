...
# Substitute: grid-wars, coworld-grid-wars, 4.
#
...
# Every PLAYER container must also have exited 0. whisky's receiveMessage
# raises on a close frame and mummy's send only queues, so the game's
# quit(0) can outrun the flushed final frame and kill a player with status
# 1 -- a race that passes one dispatch and fails the next, and that hosted
# certification reports as player_error. The starter smoke only checked the
# game (raid 0.1.3 -> 0.1.4, 2026-08-23).
player_fail=0
for ((slot = 0; slot < seats; slot++)); do
  ...
  pcode="$(docker inspect -f '{{.State.ExitCode}}' "${prefix}-p${slot}" 2>/dev/null || echo missing)"
  prunning="$(docker inspect -f '{{.State.Running}}' "${prefix}-p${slot}" 2>/dev/null || echo unknown)"
  if [ "${prunning}" = "true" ]; then
    echo "FAIL: player ${slot} was still running after the game exited" >&2
    player_fail=1
  elif [ "${pcode}" != "0" ]; then
